...
GLFWwindow *createWindow(struct windowData *data, const char *name);
void destroyWindow(GLFWwindow *window);

struct WindowManager initWindowManager(const char *name) {
    struct WindowManager result = {
        .data = calloc(1, sizeof(struct windowData))
    };

    result.window = createWindow(result.data, name);
